  ...
  function ask() {
   ...
   if (q1.test(q)) {
    ...
    msg += "<ul><li>Your name: " +
     ...
     $("#message").append(`<b>Data recieved via your IP Address</b><br>Your country: ${response.country_name}|<sup>${response.country_code}</sup>(<img src="${response.flag}" height="15vh" width="22vw">)<br>Your native language: ${response.languages[1].name}<br>Your timezone: UTC ${response.time_zone.offset} (${response.time_zone.abbr})<br>Note: We value your privacy! None of your data will be shared.`);
}, "jsonp");
    $('#message').delay(10000).animate({height:'toggle', opacity: 'toggle'}, 800);
   } else if (q4.test(q)) {
    $('#message').animate({height:'toggle', opacity: 'toggle'}, 800);
